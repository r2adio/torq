...
pnpm dev info sample.torrent

# discover peers from tracker
pnpm dev peers sample.torrent

# perform peer handshake with tracker
pnpm dev handshake sample.torrent <peer_ip>:<peer_port>

# downlaod a piece from tracker
pnpm dev downlaod_piece -o /tmp/test-piece sample.torrent <piece_index>

# download a complete file from tracker
pnpm dev download -o /tmp/out.test sample.torrent
#+end_src
