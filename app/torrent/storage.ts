...
export interface VerifyAllOptions {
  ...
  signal?: AbortSignal;
  onProgress?: (checked: number, valid: number, missing: number, corrupt: number) => void;
}
...
export class StorageManager {
  ...
  async setup(): Promise<StorageSetupSummary> {
    ...
    return {
      ...
      existingFiles,
      allFilesCreated:
        this.metadata.storageFiles.some((file) => !file.padding) && existingFiles === 0,
    };
  }
  ...
  private verifyPieceByBuffer(
    ...
    const data = this.readPieceMaybeSync(pieceIndex, tolerateMissing, readHandles);
    if (!data) return "missing";
    if (isAllZero(data) && !this.isPaddingOnlyPiece(pieceIndex)) return "missing";
  ...
  private async verifyPieceByChunks(
    ...
    for (const { file, fileOffset, length } of this.metadata.pieceToFileRanges(pieceIndex)) {
      if (file.padding) {
      ...
      const fullPath = join(this.downloadPath, file.path);
      const handle = await this.getAsyncReadHandle(fullPath, tolerateMissing, readHandles);
  ...
  private async getAsyncReadHandle(
    ...
    try {
      ...
      if (tolerateMissing && err instanceof Error && "code" in err && err.code === "ENOENT") {
        return null;
      }
      throw err;
    }
  ...
  private async closeAsyncReadHandles(readHandles: AsyncReadHandleMap): Promise<void> {
...
// yield control so the event loop can process I/O, UI, and other
// pending work — prevents long-running verification from freezing.
